...
(defn chat
  ...
  :stream -- If set, partial message deltas will be sent, like in
  ChatGPT. Tokens will be sent as data-only server-sent events as they
  ...
  in the tokenizer) to an associated bias value from -100 to
  ...
  [component & {:keys [model] :as params}]
  (make-request component
                :post "/chat/completions"
                {:form-params  (assoc params :model (or model "gpt-3.5-turbo"))
                 :content-type :json}))
